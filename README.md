...
- **Decay & GC:** exponential decay (`decay_score *= 0.95^(hours/168)`), access-count tracking, periodic garbage collection, and a user-configurable maximum memory/RAG storage cap (`AppSettings.max_memory_gb`, default 10 GB) that prunes lowest-utility memories after writes and during background maintenance
