/**
 * Brain + Local LM Studio E2E — end-to-end walkthrough of the brain complex
 * example using Local LM Studio as the brain provider.
 *
 * Uses Pinia state patching (no real LM Studio instance required) to simulate
 * the full flow: fresh launch → learn-docs intent → prerequisite check →
 ...
 *  17. Skill tree — quests activated
 *  18. Final state
 */
...
const SCREENSHOT_DIR = 'instructions/screenshots/local-lm';

// ─── Pinia patching helpers ─────────────────────────────────────────────────

async function setPinia(page: import('@playwright/test').Page, patch: Record<string, unknown>) {
  await page.evaluate((data) => {
    ...
    for (const [store, values] of Object.entries(data)) {
      ...
      if (s) {
        // For stores like settings where the main state is a ref called
        // the same name as the store key, we need to patch correctly.
        // Pinia $patch merges at the top level of the store's $state.
...
test.describe('Brain + Local LM Studio — Complete Walkthrough', () => {
  test(
    ...
    async ({ page }) => {
      ...
      // ── 01. Fresh Launch ──────────────────────────────────────────────
      // Configure brain state and enable chatbox mode via direct store manipulation
      await setPinia(page, BRAIN_LM_STUDIO_STATE);
      // Switch to chatbox mode by clicking the "Chat" mode toggle
      ...
      if (await chatModeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        // Fallback: set chatbox_mode directly on the settings store
        await page.evaluate(() => {
          const app = (document.querySelector('#app') as any)?.__vue_app__;
          const pinia = app?.config?.globalProperties?.$pinia;
          if (!pinia) return;
      ...
      await page.screenshot({ path: `${SCREENSHOT_DIR}/01-fresh-launch.png` });
      ...
      await setMessages(page, [
        {
          id: 'u1',
          role: 'user',
          content: 'Learn Vietnamese laws using my provided documents using Local LM',
          timestamp: NOW - 60000,
        },
      ]);
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('Learn Vietnamese laws', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/02-alice-learn-request.png` });
      ...
      await setMessages(page, [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            'To learn **Vietnamese laws** from your documents using Local LM Studio I need a few quests to be active first:\n\n' +
          ...
          questChoices: [
            {
              label: 'Install all',
              value: 'learn-docs:install-all:Vietnamese%20laws',
              icon: '⚡',
            },
            {
              label: 'Install one by one',
              value: 'learn-docs:install-each:Vietnamese%20laws',
              icon: '📋',
            },
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('Awaken the Mind', { exact: false }).first(),
      ).toBeVisible();
      await expect(
        page.locator('.chat-view').getByText('Install all', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/03-missing-prereqs.png` });
      ...
      await setMessages(page, [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            '\u2705 **Awaken the Mind** activated \u2014 Local LM Studio connected (gemma-4-12b-it @ http://127.0.0.1:1234)',
          agentName: 'System',
          sentiment: 'happy',
          timestamp: NOW - 54000,
        },
        {
          id: 'q2',
          role: 'assistant',
          content: '✅ **Long-Term Memory** activated — SQLite memory store online',
          agentName: 'System',
          sentiment: 'happy',
          timestamp: NOW - 53000,
        },
        {
          ...
          role: 'assistant',
          content:
            "\u2705 **Sage's Library** activated \u2014 6-signal hybrid RAG pipeline ready (qwen3-embedding-0.6b embeddings)",
          agentName: 'System',
          sentiment: 'happy',
          timestamp: NOW - 52000,
        },
        {
          id: 'q4',
          role: 'assistant',
          content: "✅ **Scholar's Quest** activated — document ingestion unlocked",
          agentName: 'System',
          sentiment: 'happy',
          timestamp: NOW - 51000,
        },
      ...
      await expect(
        page.locator('.chat-view').getByText('All 4 quests installed', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/04-auto-install.png` });

      // ── 05. Brain tab — LM Studio configured ─────────────────────────
      await navigateToTab(page, 'Brain');
      await setPinia(page, { ...BRAIN_LM_STUDIO_STATE, ...MEMORY_STATE });
      await page.waitForTimeout(1200);
      await expect(
        page.locator('.brain-view').getByText('Your brain is alive', { exact: false }).first(),
      ).toBeVisible({ timeout: 5000 });
      await page.screenshot({ path: `${SCREENSHOT_DIR}/05-brain-configured.png` });
      ...
      await setMessages(page, [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            "📚 **Scholar's Quest — Step 1: Gather Sources**\n\n" +
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('Gather Sources', { exact: false }).first(),
      ).toBeVisible();
      await expect(
        page
          .locator('.chat-view')
          .getByText('vietnamese-civil-code.html', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/06-attach-documents.png` });
      ...
      await setMessages(page, [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            "📚 **Scholar's Quest — Step 2: Learning in Progress**\n\n" +
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('Learning in Progress', { exact: false }).first(),
      ).toBeVisible();
      await expect(
        page.locator('.chat-view').getByText('15 memories created', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/07-ingestion-progress.png` });

      // ── 08. Memory tab — 15 knowledge entries ─────────────────────────
      await navigateToTab(page, 'Memory');
      await setPinia(page, MEMORY_STATE);
      await page.waitForTimeout(600);
      await expect(page.locator('.memory-view')).toBeVisible({ timeout: TIMEOUTS.panel });
      await expect(
        page.locator('.memory-view').getByText('Memory', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/08-memory-tab.png` });
      ...
      await setMessages(page, [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            'Based on your imported Vietnamese Civil Code documents:\n\n' +
      ...
      await page.screenshot({ path: `${SCREENSHOT_DIR}/09-rag-answer.png` });
      ...
      await setMessages(page, [
        {
          id: 'u-rag',
          role: 'user',
          content: 'What is the statute of limitations for contract disputes?',
          timestamp: NOW - 30000,
        },
        {
          id: 'a-rag',
          role: 'assistant',
          content:
            'Article 429 sets the limitation at **three years** from the date the claimant knew of the breach.\n\n> 📚 Sources: `vietnamese-civil-code.html`, `article-429-commentary.txt`',
          agentName: 'TerranSoul',
          sentiment: 'neutral',
          timestamp: NOW - 25000,
        },
        {
          id: 'u2',
          role: 'user',
          content: 'Can a party claim both a penalty and damages for breach of contract?',
          timestamp: NOW - 20000,
        },
        {
          id: 'a2',
          role: 'assistant',
          content:
            'Yes! Under **Article 420** of the Vietnamese Civil Code 2015:\n\n' +
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('Article 420', { exact: false }).first(),
      ).toBeVisible();
      await expect(
        page.locator('.chat-view').getByText('may claim both', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/10-followup-answer.png` });
      ...
      await setMessages(page, [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            'Theo tài liệu Bộ luật Dân sự Việt Nam của bạn:\n\n' +
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('Điều 429', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/11-vietnamese-answer.png` });
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('第429条', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/12-chinese-answer.png` });
      ...
      await setMessages(page, [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            'Согласно импортированным документам Гражданского кодекса Вьетнама:\n\n' +
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('Статья 429', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/13-russian-answer.png` });
      ...
      await setMessages(page, [
        {
          id: 'u-ja',
          role: 'user',
          content: 'ベトナム法における契約紛争の出訴期限はどのくらいですか？',
          timestamp: NOW - 10000,
        },
        {
          id: 'a-ja',
      ...
      await page.screenshot({ path: `${SCREENSHOT_DIR}/14-japanese-answer.png` });
      ...
      await setMessages(page, [
        {
          id: 'u-ko',
          role: 'user',
          content: '베트남 법률에서 계약 분쟁의 소멸시효는 얼마입니까?',
          timestamp: NOW - 10000,
        },
        {
          id: 'a-ko',
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('제429조', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/15-korean-answer.png` });
      ...
      await page.screenshot({ path: `${SCREENSHOT_DIR}/16-brain-rag-active.png` });

      // ── 17. Skill tree ────────────────────────────────────────────────
      await navigateToTab(page, 'Quests');
      await page.waitForTimeout(1000);
      await page.screenshot({ path: `${SCREENSHOT_DIR}/17-skill-tree.png` });
      ...
      await setMessages(page, [
        {
          id: 'u-final',
          role: 'user',
          content: 'Summarize what you know about me and my documents.',
          timestamp: NOW - 5000,
        },
        {
          id: 'a-final',
          role: 'assistant',
          content:
            "Here's what I know about you and your study materials:\n\n" +
            '**About you:**\n' +
      ...
      await page.waitForTimeout(600);
      await expect(
        page.locator('.chat-view').getByText('law student', { exact: false }).first(),
      ).toBeVisible();
      await expect(
        page.locator('.chat-view').getByText('15 memories', { exact: false }).first(),
      ).toBeVisible();
      await expect(
        page.locator('.chat-view').getByText('Local LM Studio', { exact: false }).first(),
      ).toBeVisible();
      await page.screenshot({ path: `${SCREENSHOT_DIR}/18-final.png` });

      // ── Verify brain mode is LM Studio throughout ─────────────────────
      const brainState = (await getPiniaState(page, 'brain')) as any;
      expect(brainState?.brainMode?.mode).toBe('local_lm_studio');
      expect(brainState?.brainMode?.base_url).toBe('http://127.0.0.1:1234');
      expect(brainState?.brainMode?.model).toBe('gemma-4-12b-it');
