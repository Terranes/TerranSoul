...
> **Status — Phase 15 mostly shipped (May 2026).** Shipped: `BrainGateway`
> trait (15.3), MCP HTTP server (15.1), gRPC `brain.v1` transport foundation
> (15.2), auto-setup writers (15.6), MCP stdio transport (15.9), VS Code
> workspace surfacing (15.10), voice/chat intents (15.5). Remaining: Control
> Panel Vue view (15.4), incremental-indexing QA (15.7), final doc pass
> (15.8). Sections marked **Planned** are not yet wired up and will be filled
> in (with screenshots, exact command names, and CLI samples) as each chunk
> lands.
...
- Schema: `src-tauri/proto/terransoul/brain.v1.proto`.
- Build: `src-tauri/build.rs` uses vendored `protoc` plus `tonic-prost-build`
  so CI does not depend on a host-level protobuf compiler.
- Crates: `tonic` + `prost` + `tonic-prost-build`. **Recommended over MCP** for
...
- Bind: `127.0.0.1:7422` by default (`ai_integrations::grpc::DEFAULT_GRPC_ADDR`).
- Auth: accepts a `ServerTlsConfig` built from PEM material via
  `tls_config_from_pem`; providing a client CA enables tonic/rustls mTLS.
  The server helper refuses plaintext on non-loopback addresses, so future LAN
  activation cannot accidentally expose the brain without TLS.
- Surface: `Health`, `Search`, `StreamSearch`, `GetEntry`, `ListRecent`,
  `KgNeighbors`, `Summarize`, `SuggestContext`, and `IngestUrl`, all delegated
  to the shared `BrainGateway` capability gate.
- Runtime startup: explicit caller-owned `serve_with_shutdown`; Control Panel
  start/stop wiring remains Chunk 15.4 and LAN bind/cert-registry wiring remains
  Phase 24.
...
| 15.2 | ✅ shipped 2026-05-01 | gRPC `brain.v1` transport foundation (`tonic`) with mTLS-capable config + streaming search |
