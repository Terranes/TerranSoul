...
│ Benchmark writes measured reports to │
...
the pre-release migration runner into a single canonical
...
4. Creates `contains` edges from root → each . Creates `follows` edges between consecutive chunks (preserving document order)
...
│ from cognitive_kind, edge color from rel_type).5 │
│ │
