...
pub enum InstallMethod {
    ...
    /// An agent that is **compiled into TerranSoul itself** (for example the
    /// reference [`crate::agent::stub_agent::StubAgent`]). Built-in agents have
    /// no binary to download — installation only writes the manifest so the
    /// orchestrator can list/enable/disable them like any other agent. Chat
    /// tools such as OpenClaw should use the plugin manifest path instead.
    BuiltIn,
}
