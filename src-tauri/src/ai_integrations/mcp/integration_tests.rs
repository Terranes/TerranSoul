...
mod tests {
    ...
    async fn tools_list_returns_28_tools() {
        ...
        // CTX-OFFLOAD-1a inserts brain_drilldown_payload right after that.
        assert_eq!(tools[3]["name"], "brain_kg_neighbors");
        assert_eq!(tools[4]["name"], "brain_drilldown");
        assert_eq!(tools[5]["name"], "brain_drilldown_payload");

        // Verify Knowledge Wiki tools are present before code tools.
        assert_eq!(tools[13]["name"], "brain_wiki_audit");
        assert_eq!(tools[17]["name"], "brain_wiki_digest_text");
        assert_eq!(tools[18]["name"], "brain_review_gaps");
        assert_eq!(tools[19]["name"], "brain_session_checklist");

        // Verify repo RAG tools (BRAIN-REPO-RAG-1c-b-i) are between brain and code.
        assert_eq!(tools[20]["name"], "repo_search");
        assert_eq!(tools[21]["name"], "repo_list_files");
        assert_eq!(tools[22]["name"], "repo_read_file");
        // Cross-source All fan-out (BRAIN-REPO-RAG-1c-b-ii-a).
        assert_eq!(tools[23]["name"], "cross_source_search");
        // Aider-style repo map + signatures (BRAIN-REPO-RAG-1d).
        assert_eq!(tools[24]["name"], "repo_map");
        assert_eq!(tools[25]["name"], "repo_signatures");

        // Verify code tools are present.
        assert_eq!(tools[26]["name"], "code_query");
        assert_eq!(tools[29]["name"], "code_rename");
        assert_eq!(tools[39]["name"], "code_branch_sync");
        assert_eq!(tools[40]["name"], "code_index_commit");
        assert_eq!(tools[41]["name"], "code_branch_diff");
        assert_eq!(tools[42]["name"], "code_group_drift");
