...
- When a session learns a durable rule, convention, architecture fact, or reverse-engineering lesson, sync it into TerranSoul MCP self-improve knowledge in the same PR by updating `mcp-data/shared/**` (usually `memory-seed.sql`, `lessons-learned.md`, or `project-index.md`) and, when MCP is running, using the brain tools to verify it is retrievable.
